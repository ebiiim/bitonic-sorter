use bitonic_sorter::utils::{is_sorted, new_u32_vec};
use bitonic_sorter::v3::sort as seq_sort;
use bitonic_sorter::v4::sort as par_sort;
...
fn main() {
    if let Some(n) = env::args().nth(1) {
        ...
        eprintln!("Usage {} NUMS_LEN_BITS", env::args().nth(0).unwrap());
        std::process::exit(1);
    }
}

fn run_sorts(bits: u32) {
    let len: usize = 1 << bits;
    println!(
        "sorting {} integers ({:.1} MB)",
        len,
        (len * std::mem::size_of::<u32>()) as f64 / (1 << 20) as f64
    );
    println!(
