    ...
    F: Sync + Fn(&T, &T) -> Ordering,
{
    if x.len().is_power_of_two() {
        ...
        Err(format!("{} is not a power of two", x.len()))
    }
}
...
{
    if x.len() > 1 {
        let mid_point = x.len() >> 1;
        let (a, b) = x.split_at_mut(mid_point);
        if mid_point >= PARALLEL_THRESHOLD {
            rayon::join(
                || do_sort(a, true, comparator),
                || do_sort(b, false, comparator),
            );
        } else {
            do_sort(a, true, comparator);
            do_sort(b, false, comparator);
    ...
    F: Sync + Fn(&T, &T) -> Ordering,
{
    if x.len() > 1 {
        ...
        if mid_point >= PARALLEL_THRESHOLD {
            rayon::join(
                || sub_sort(a, forward, comparator),
                || sub_sort(b, forward, comparator),
            );
        } else {
            sub_sort(a, forward, comparator);
            sub_sort(b, forward, comparator);
    ...
    F: Fn(&T, &T) -> Ordering,
{
    ...
    for i in 0..mid_point {
        if comparator(&x[i], &x[mid_point ^ i]) == swap_cond {
